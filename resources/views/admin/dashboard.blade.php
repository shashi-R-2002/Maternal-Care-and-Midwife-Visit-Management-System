    ...
    <div class="content">
        ...
        <div class="stats-grid">
            <div class="stat-card amber">
                ...
                <div class="stat-body"><div class="s-label">Pending Requests</div><div class="s-value" id="countPending">{{ $pendingCount }}</div></div>
            </div>
            <div class="stat-card green">
                <div class="stat-icon"><i class="bi bi-person-check-fill"></i></div>
                <div class="stat-body"><div class="s-label">Approved</div><div class="s-value" id="countApproved">{{ $approvedCount }}</div></div>
            </div>
            <div class="stat-card red">
                <div class="stat-icon"><i class="bi bi-person-x-fill"></i></div>
                <div class="stat-body"><div class="s-label">Rejected</div><div class="s-value" id="countRejected">{{ $rejectedCount }}</div></div>
            </div>
            <div class="stat-card blue">
                <div class="stat-icon"><i class="bi bi-people-fill"></i></div>
                <div class="stat-body"><div class="s-label">Total Requests</div><div class="s-value" id="countTotal">{{ $totalCount }}</div></div>
            </div>
        </div>
        ...
        <div class="box">
            ...
            <div class="filter-pills">
                <div class="filter-pill active" data-filter="all">All <span class="pill-count" id="pill-all">{{ $totalCount }}</span></div>
                <div class="filter-pill" data-filter="pending">Pending <span class="pill-count" id="pill-pending">{{ $pendingCount }}</span></div>
                <div class="filter-pill gpill" data-filter="approved">Approved <span class="pill-count" id="pill-approved">{{ $approvedCount }}</span></div>
                <div class="filter-pill rpill" data-filter="rejected">Rejected <span class="pill-count" id="pill-rejected">{{ $rejectedCount }}</span></div>
            </div>
...
<tr data-status="{{ strtolower($midwife->status) }}">

    <td style="color:#94a3b8;font-size:12px;font-weight:600;">
        {{ $midwife->registration_no ?? '-' }}
    </td>
    ...
    <td>
        <div class="midwife-cell">
            ...
            <div class="m-avatar">
                {{ strtoupper(substr($midwife->full_name,0,1)) }}
            </div>

            <div>
                <div class="m-name">{{ $midwife->full_name }}</div>
                <div class="m-email">{{ $midwife->email }}</div>
            </div>
    ...
    <td>
        <span class="date-cell">
            ...
            {{ $midwife->created_at->format('d M Y') }}
        </span>
    </td>
    ...
    <td>
        ...
        @elseif($midwife->status=='Rejected')
    ...
    <td>
        ...
        <div class="action-group">
            ...
            @if($midwife->status=='Pending')
                ...
                <form action="{{ route('admin.midwives.approve',$midwife) }}"
                      method="POST"
                ...
                <form action="{{ route('admin.midwives.reject',$midwife) }}"
                      method="POST"
                      style="display:inline;">
                    @csrf
            ...
            @elseif($midwife->status=='Approved')
...
<strong id="visibleCount">{{ $midwives->count() }}</strong>
of

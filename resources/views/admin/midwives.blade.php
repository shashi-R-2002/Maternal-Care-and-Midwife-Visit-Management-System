...
<div class="sidebar">
    ...
    <span class="nav-icon"><i class="bi bi-speedometer2"></i></span>Dashboard
        ...
        <a href="{{ route('admin.midwives') }}"class="active">
...
<tr>
    <td>{{ $midwife->registration_no ?? '-' }}</td>

    <td>
        <strong>{{ $midwife->full_name }}</strong><br>
        <small>{{ $midwife->email }}</small>
    </td>
    ...
    <td>
        {{ optional($midwife->updated_at)->format('d M Y') }}
    </td>
...
@endforelse
    ...
    <span>
        ...
        of
        <strong id="totalCount">{{ $midwives->count() }}</strong>
        approved midwives
    </span>
